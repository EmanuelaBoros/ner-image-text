...
class Imagine(nn.Module):
    ...
    def set_text(self, text, out_name=None):
        self.out_name = out_name
    ...
    def set_clip_encoding(self, text=None, img=None, encoding=None, text_min=""):
        ...
        if self.out_name is None:
            final_name = text_path
        else:
            final_name = self.out_name
        self.filename = Path(os.path.join(self.out_path, f'./{final_name}{self.seed_suffix}.png'))
    ...
    def train_step(self, epoch, i, pbar=None):
        ...
        if (i + 1) % self.save_every == 0:
            with torch.no_grad():
                ...
                #print("Top score", top_score.item(), "-- current best score", self.current_best_score)
                ...
                if self.save_progress:
                    ...
                    save_image(image, Path(os.path.join(self.out_path, f'./{final_name}.{num}{self.seed_suffix}.png')))

                if self.save_best and top_score.item() < self.current_best_score:
                    self.current_best_score = top_score.item()
                    save_image(image, Path(os.path.join(self.out_path, f'./{final_name}{self.seed_suffix}.best.png')))
                    self.worst_counter = 0

                if top_score.item() > self.current_best_score:
                    self.worst_counter += 1
                    #print(f"Can't improve image for {self.worst_counter} times.")
                if self.worst_counter == self.worst_limit:
                    #print("Finishing!")
                    return  out, total_loss, True

        return out, total_loss, False
    ...
    def forward(self):
        ...
        print(f'Imagining "{self.text_path}" {penalizing} in {self.filename}...')
        ...
        for epoch in trange(self.epochs, desc = '      epochs', position=0, leave=True):
            ...
            image_pbar.update(0)
            for i in pbar:
                out, loss, finish = self.train_step(epoch, i, image_pbar)
