...
#ALL_MODELS = sum(
#    (tuple(conf.pretrained_config_archive_map.keys()) for conf in (AutoConfig, AutoConfig)),
#    ())

MODEL_CLASSES = {
    "bert": (AutoConfig, BertForTokenClassification_, AutoTokenizer),
    "roberta": (AutoConfig, BertForTokenClassification_, AutoTokenizer)
}
...
# def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix="", language_code="en"):
def evaluate(args, model, eval_dataset, labels, pad_token_label_id, mode, prefix="", language_code="en"):
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode, language_code):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        examples = read_examples_from_file(args.data_dir, language_code, mode)
        # import pdb;pdb.set_trace()
...
#    src_model_path = os.path.join(args.src_model_dir)
    print('FROM:', args.src_model_dir)
    src_model = model_class.from_pretrained(args.src_model_dir)
#    model = model_class.from_pretrained(args.output_dir)
#    tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path)
#    src_model = AutoModel.from_pretrained(model_class)
    ...
    logger.info("***** Compute logits for [%s] dataset using the model [%s] *****", os.path.basename(args.data_dir), model_class)
...
def main():
    ...
    parser.add_argument("--language_code", default="en", type=str,
                        help="language_code")
    parser.add_argument("--language_code_test", default="en", type=str,
                        help="language_code")

    parser.add_argument("--context", default="", type=str,
                        help="_context_2")
...
#            raise ValueError("Predict: Output directory ({}) already exists and is not empty.".format(args.output_dir))
    ...
    # Training
    if args.do_train:
#      if args.do_continue and args.local_rank in [-1, 0]:
...
#      else:
      ...
      print('MODEL TYPE:', type(model), args.model_name_or_path)
      ...
      # train_dataset = Subset(train_dataset, train_indices)#.dataset
      # eval_dataset = Subset(train_dataset, test_indices)#.dataset
      
      print(len(train_dataset), len(eval_dataset))
      # # train_ds, eval_ds = torch.utils.data.random_split(train_dataset, (int(80/100*len(train_dataset)), int(20/100*len(train_dataset))))
      # # eval_dataset = None
      # import pdb;pdb.set_trace()
      # # eval_dataset = load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode=mode, language_code=language_code)
      ...
      if args.do_KD:
          ...
          if args.hard_label:
              ...
              global_step, tr_loss = train(args, model, train_dataset, eval_dataset, tokenizer, labels, pad_token_label_id)
...
#        import pdb;pdb.set_trace()
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        result, predictions = evaluate(args, model, tokenizer, labels, pad_token_label_id, 
                                       mode="test"+args.context, 
                                       language_code=args.language_code_test)
        ...
        with open(output_test_predictions_file, "w", encoding='utf-8') as writer:
            with open(os.path.join(args.data_dir, "{}_{}.conll".format(args.language_code, "test"+args.context)), "r", encoding='utf-8') as f:
                example_id = 0
                for line in f:
